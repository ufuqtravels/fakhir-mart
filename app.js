...
// Initialize Local/Cloud Database (Resilient Caching)
async function initDatabase() {
  ...
  // ২. ক্যাশ থেকে প্রোডাক্ট ও অর্ডার দ্রুত স্ক্রিনে লোড হবে
  ...
  // যদি ক্যাশ খালি থাকে, তবে products.js ফাইলের ব্যাকআপ ডাটা লোড হবে
  if (products.length === 0 && typeof INITIAL_PRODUCTS !== 'undefined') {
  ...
  // ৩. ব্যাকগ্রাউন্ডে ক্লাউড থেকে রিয়েল-টাইমে ডাটা নামানো হচ্ছে
  ...
  try {
    ...
    if (data.record) {
      ...
      if (Array.isArray(data.record)) {
        // যদি এটি সাধারণ লিস্ট ফরম্যাট হয়
        products = data.record;
      } else {
        // যদি এটি নতুন অবজেক্ট ফরম্যাট হয়
...
// Initialize Local/Cloud Database (Resilient Caching)
async function initDatabase() {
  // ১. কার্ট স্থানীয় ব্রাউজার মেমোরি থেকে লোড হবে
  const localCart = localStorage.getItem('fm_cart');
  if (localCart) cart = JSON.parse(localCart);
  ...
  // যদি ক্যাশ খালি থাকে, তবে products.js ফাইলের ব্যাকআপ ডাটা লোড হবে (কখনো ব্লাঙ্ক দেখাবে না)
  if (products.length === 0 && typeof INITIAL_PRODUCTS !== 'undefined') {
    products = [...INITIAL_PRODUCTS];
  }
...
async function processOrderPlacement(subtotal, delivery, total) {
  ...
  // রিয়েল-টাইম ক্লাউড সিঙ্কসহ অর্ডার লিস্ট আপডেট
  orders.push(newOrder);
  saveOrdersToLocal();
